# -*- coding: utf-8 -*-
"""
Created on Sun Oct  1 21:14:44 2023
...
raoa=pd.read_csv('D:/API/elab_psofeat.csv')
feat=raoa.drop(columns='Label', axis=1)
label=raoa['Label']
X_train, X_test, Y_train, Y_test = train_test_split(feat, label, test_size=0.3, stratify=label, random_state=23)
loadmodel     = XGBClassifier(colsample_bytree=0.7,
                        gamma=0.0,
                        learning_rate=0.05, 
                        max_depth=6,
                        min_child_weight=1,  
                        n_estimators=50, 
                        subsample=1.0, eval_metric='logloss',objective='binary:logistic')
loadmodel.fit(X_train, Y_train)

Y_train_predict=loadmodel.predict(X_train)
train_acc=accuracy_score(Y_train, Y_train_predict)
print('Accuracy Score of the training data:', train_acc)

Y_test_predict=loadmodel.predict(X_test)
test_acc=accuracy_score(Y_test, Y_test_predict)
print('Accuracy Score of the testing data:', test_acc)
